...
#Criar 2 colunas para caber os gráficos um do lado do outro
col1, col2 = st.columns(2)
df3 = df2.groupby('placar')[['home_team_name']].count().reset_index().rename(columns={'home_team_name':'qtd'})
print(df3)
figpizza = px.pie(data_frame=df3, values='qtd', names='placar')
figcaixa = px.box(df2, x='home_ppg')
col1.plotly_chart(figpizza, use_container_width=True) #ajustar o tamanho (use_container_width)
